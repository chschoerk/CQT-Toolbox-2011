...
for i=1:octaveNr  
    ...
    XX = 2.* XX ./ cqtKernel.fftLEN; %**CHANGED!
    for n=1:2^(i-1) 
        shift = (n-1)*inc;
...
emptyHops = cqtKernel.firstcenter/cqtKernel.atomHOP;
maxDrop = emptyHops*2^(octaveNr-1)-emptyHops;
droppedSamples = (maxDrop-1)*cqtKernel.atomHOP + cqtKernel.firstcenter;
outputTimeVec = (1:size(spCQT,2))*cqtKernel.atomHOP-prefixZeros+droppedSamples;
outputTimeVec = outputTimeVec./fs;
...
intParam = struct('sufZeros',suffixZeros,'preZeros',prefixZeros,'xlen_init',xlen_init,'fftLEN',cqtKernel.fftLEN,'fftHOP',cqtKernel.fftHOP,...
    ...
    'atomNr',cqtKernel.atomNr,'Nk_max',cqtKernel.Nk_max,'Q',cqtKernel.Q,'rast',1);%'weight',cqtKernel.weight); %**CHANGED!! ('weight' added));

Xcqt = struct('spCQT',spCQT,'fKernel',cqtKernel.fKernel,'fmax',fmax,'fmin',fmin,'octaveNr',octaveNr,...
    'timeVector',outputTimeVec, 'bins',cqtKernel.bins,'intParams',intParam); %**CHANGED!! ('timeVector' added)